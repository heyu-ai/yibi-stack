...
- **protect-push 攔截 `gh pr merge`**：agent 無法自行 merge；需使用者執行 `! gh pr merge <n> --squash --delete-branch`。另：在 linked worktree 內執行 `gh pr merge` 時若主 repo 已 checkout main 也會失敗（`fatal: 'main' is already used by worktree`）——同樣需從主 repo 目錄執行
