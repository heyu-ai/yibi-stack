...
所有錨點比對 **MUST** 以 `Path.read_text(encoding="utf-8")` 讀原始 UTF-8。錨點含
...
|--------------|---------|-----------|---------|-------|
| `complete-contract-starts-review` | △ partial | DT | PRC-DT-005 | doc 層釘住五段標題在場；「凍結後啟動 R1」為執行期行為，不可測 |
| `missing-contract-section-blocks-reviewer-launch` | ✓ | DT / RB | PRC-DT-005, PRC-EG-006 | 完全機械化：移除任一標題錨點使檢查器變紅（DT-005 直接斷言、EG-006 mutation 兜底） |
| `acceptance-criterion-violation-can-block` | △ partial | DT | PRC-DT-006 | doc 層釘住 `Contract mapping:` 錨點；映射到 AC 後 blocking 為執行期行為 |
| `missing-mapping-is-non-blocking` | △ partial | DT | PRC-DT-006 | doc 層釘住 `blocking set is the sole LGTM gate`；實際降級為執行期行為 |
...
| `risk-inside-accepted-boundary-is-non-blocking` | △ partial | DT | PRC-DT-006 | doc 層釘住 `Accepted by:` 錨點；風險認領為執行期 |
| `missing-acceptor-leaves-risk-unaccepted` | △ partial | DT | PRC-DT-006 | doc 層釘住 `Accepted by:`；缺 acceptor 視為未接受屬執行期判斷 |
| `non-blocking-needs-changes-does-not-veto-merge` | △ partial | DT | PRC-DT-006 | doc 層釘住 `blocking set is the sole LGTM gate`；raw verdict 無否決權屬執行期 |
| `blocking-set-prevents-lgtm` | △ partial | DT | PRC-DT-006 | doc 層；同上 |
| `clean-round-1-skips-cross-debate` | ✓ | DT | PRC-DT-006 | 完全機械化：`R2 skipped: no contract-blocking candidate or dispute` 錨點缺失即變紅 |
| `candidate-blocker-activates-cross-debate` | △ partial | DT | PRC-DT-006 | doc 層釘住 conditional-R2 錨點；實際啟動為執行期 |
| `blocking-disagreement-activates-cross-debate` | △ partial | DT | PRC-DT-006 | doc 層；同上 |
| `material-amendment-restarts-review` | ✓ | DT | PRC-DT-006 | 完全機械化：`material amendment` 錨點缺失即變紅 |
| `editorial-correction-keeps-current-pass` | ✓ | DT | PRC-DT-006 | 完全機械化：`editorial amendment` 錨點缺失即變紅 |
| `required-heading-mutation-fails` | ✓ | DT / RB | PRC-DT-005, PRC-EG-006 | 完全機械化 |
...
|-------|-------------|-----------|------|-------------|-------|-----------|----------------|
| PRC-DT-005 | [doc] Review Contract 五段標題缺任一 → 檢查器變紅 | DT | High | 真實 SKILL.md；純函式可用 | parametrize 6 個標題，各自從 fixture 移除後呼叫檢查器 | `## Review Contract`、`### Goal`、`### Non-goals`、`### Accepted Residual Risks`、`### Acceptance Criteria`、`### Follow-ups` | 每個標題移除後，失敗訊息含該標題與 `absent`；絕不 `[]` |
| PRC-DT-006 | [doc] 契約決策錨點缺任一 → 檢查器變紅 | DT | High | 同上 | parametrize 7 個錨點，各自移除後呼叫 | `frozen Review Contract`、`Contract mapping:`、`Accepted by:`、`blocking set is the sole LGTM gate`、`R2 skipped: no contract-blocking candidate or dispute`、`material amendment`、`editorial amendment` | 每個錨點移除後失敗並指名 + `absent` |
...
- **contract 凍結後才啟動 R1、material amendment 重啟 full-diff R1**——`PRC-DT-005/006` 只證
  runbook **有寫**這些規則，無法證 lead 真的凍結／重啟。屬 `[doc]`。
...
| Requirement | Scenario slug | TC-ID | pytest docstring |
|----|--------------|-------|-----------------|
