...
`REQUIRED_ANCHORS`（Review Contract 六個標題：`## Review Contract` wrapper + 5 段 + 契約決策錨點）、`FORBIDDEN_STRINGS`
...
### Unicode 讀取要求（Python 層原始 UTF-8 讀取）

所有錨點比對 **MUST** 以 `Path.read_text(encoding="utf-8")` 讀原始 UTF-8（Python 層讀取慣例，
非 bash 層議題,故不掛靠任何 rule 13 子節）。錨點含 `每個 PR 至多一張` 等全形／CJK 字元;
用 shell `echo` 轉換或 ASCII 替代會**靜默不匹配**、不報錯，導致整批檢查 no-op。
...
|--------------|---------|-----------|---------|-------|
| `complete-contract-starts-review` | △ partial | DT | PRC-DT-005 | 錨點在場（六個標題）為機械層；「凍結後啟動 R1」為執行期行為，不可測 |
...
|-------|-------------|-----------|------|-------------|-------|-----------|----------------|
| PRC-DT-005 | [mech] Review Contract 六個標題（`## Review Contract` wrapper + 5 段）缺任一 → 檢查器變紅（對合成 fixture，非真實檔案；真實檔案由 EG-006/SMK-001 兜底） | DT | High | 純函式（`_fixture(900)` 合成字串，不讀真實 SKILL.md） | parametrize 6 個標題，各自從 fixture 移除後呼叫檢查器 | `## Review Contract`、`### Goal`、`### Non-goals`、`### Accepted Residual Risks`、`### Acceptance Criteria`、`### Follow-ups` | 每個標題移除後，失敗訊息含該標題與 `absent`；絕不 `[]` |
| PRC-DT-006 | [mech] 契約決策錨點缺任一 → 檢查器變紅（**跨 6 個 requirement**：見 Traceability Matrix 逐錨點對照） | DT | High | 純函式（合成 fixture，不讀真實檔案） | parametrize 7 個錨點，各自移除後呼叫 | `frozen Review Contract`、`Contract mapping:`、`Accepted by:`、`blocking set is the sole LGTM gate`、`R2 skipped: no contract-blocking candidate or dispute`、`material amendment`、`editorial amendment` | 每個錨點移除後失敗並指名 + `absent` |
...
**PRC-DT-006 逐錨點 → requirement 對照**（DT-006 parametrize 7 錨點，跨 6 個 requirement；docstring 只帶
