...
### BVA: token budget — exactly at budget

- **GIVEN** `token_budget=500`, and the first 3 lessons total exactly 500 tokens
- **WHEN** `get_lessons(token_budget=500)` is called
- **THEN** all 3 lessons are returned (exactly at budget is not over; inclusion is correct)

### BVA: token budget — one token over budget

- **GIVEN** `token_budget=500`, first 3 lessons total 480 tokens, 4th lesson is 21 tokens (total 501)
- **WHEN** `get_lessons(token_budget=500)` is called
- **THEN** only the first 3 lessons are returned (adding the 4th would exceed 500)

### BVA: token budget = 0 (unlimited)

- **GIVEN** `token_budget=0` (the default)
- **WHEN** `get_lessons()` is called with 100 lessons in DB
- **THEN** all lessons satisfying the tier/mode filter are returned with no token limit applied

### EP: MemoryIndex FTS5-only fallback when sqlite-vec unavailable

- **GIVEN** sqlite-vec extension is not loadable in the current SQLite build
- **WHEN** `SqliteVecIndex` is initialized
- **THEN** a WARNING is logged, initialization succeeds, and `search(mode="keyword")` operates
  using FTS5 without raising an exception

### BVA: hybrid search RRF merge score verification (k=60)

- **GIVEN** lesson L1 ranks 1st in FTS5 and 2nd in vector search
- **WHEN** RRF score is computed
- **THEN** L1's RRF score = 1/(61) + 1/(62) = 0.01639 + 0.01613 = 0.03252

- **GIVEN** lesson L2 ranks 3rd in FTS5 and 1st in vector search
- **WHEN** RRF score is computed
- **THEN** L2's RRF score = 1/(63) + 1/(61) = 0.01587 + 0.01639 = 0.03226
- **AND** L1 (0.03252) ranks above L2 (0.03226) in the merged result

### EP: invalid mode value

- **GIVEN** `get_lessons(mode="invalid_mode")` is called
- **WHEN** the call is processed
- **THEN** a `ValueError` is raised listing valid modes: episodic, semantic, procedural

### EP: MemoryIndex delete removes from search results

- **GIVEN** a lesson with id="L1" has been upserted into the index
- **WHEN** `delete("L1")` is called followed by `search("L1 content")`
- **THEN** "L1" does not appear in the search results
