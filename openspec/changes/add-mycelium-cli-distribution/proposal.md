...
- **AC-001-3**：GIVEN 使用者閱讀 English 或繁體中文 README install section 或任一六個 skill failure gate，WHEN 尋找安裝方式，THEN MUST 看見 plugin 與 CLI 兩條不同目的的互補軌，CLI MUST 只使用 exact Git-tag command，且 MUST NOT 出現 PyPI install command。
...
- **AC-002-1**：GIVEN PATH 中沒有 `mycelium`，WHEN 任一六個 skill 開始第一個 tasks-backed operation，THEN skill MUST 先輸出 `[FAIL]` 與 exact Git-tag command、非零退出，且 MUST NOT 嘗試 `SKILL_REPO`、`uv run`、`uvx` 或 `python -m tasks.*`。
...
- **AC-002-4**：GIVEN PATH 中有 installed `mycelium`，WHEN install-hooks 註冊 auto-handover 或 checkout wrapper 處理支援的 payload，THEN settings commands MUST 固定使用 registration-time resolved absolute path，wrappers MUST 使用 runtime `command -v` 結果，且兩者 MUST NOT 使用 checkout import 或 `uvx`；解析失敗 MUST `[FAIL]` 並停止。
...
| # | 限制 | 來源 |
|---|------|------|
| C1 | Phase A 唯一支援字串是 `uv tool install "yibi-stack @ git+https://github.com/heyu-ai/yibi-stack@<tag>"`；README 與六個 gates 不得提供變體。 | issue #222 Gap A direction 1、design D3、spec Two-track installation documentation |
...
| C5 | settings hook 在 registration time 固定 PATH 解析出的 absolute path；checkout wrapper 在 runtime 使用 `command -v mycelium`；兩者都不得使用 `uvx` 或 checkout import。 | design D5、spec Stable installed hook binary |
...
**GIVEN** clean HOME/PATH 有 `uv` 與 `git`、可存取 GitHub、沒有 yibi-stack checkout，且 recorded immutable tag 為 `v1.11.0`
...
#### Scenario: smk-missing-binary-gate -- SMK-002 缺 binary 時先阻擋

**GIVEN** Claude Code plugin cache 可發現六個 skills，但 PATH 沒有 `mycelium`
**WHEN** 依序啟動六個 skills 的 preflight
**THEN** 每個 skill MUST 先印 `[FAIL]` 與 exact Git-tag install command、非零退出，且 MUST NOT 嘗試 `SKILL_REPO`、`uv run`、`uvx` 或 `python -m tasks.*`
...
**THEN** 兩個 command MUST 以 `/tmp/mycli-smoke/bin/mycelium` 開頭並使用正確 hook subcommand，且 MUST NOT 含 checkout path、`python -m tasks.mycelium` 或 `uvx`
