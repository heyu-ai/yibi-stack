...
[ADDED] Coverage table 新增 proposal Step 5 的三個 smoke scenario；其正式 US／TC mapping 仍只維護於本檔 Traceability Matrix。
...
[ADDED] TC table 追加 SMK-001..003；這些是 apply phase 必須轉為 executed 的 clean-environment smoke cases。
...
| SMK-001 | 驗證 Git-tag 安裝後三個 CLI 可立即使用 | SMK | High | clean HOME/PATH；uv、git 與 GitHub 可用；filesystem 無 yibi-stack checkout；recorded immutable tag 為 `v1.11.0` | 1. 以唯一 Phase A command 安裝 `v1.11.0` 2. 執行 `mycelium --help` 3. 執行 `pr-orchestrator --help` 4. 執行 `portman --help` | `uv tool install "yibi-stack @ git+https://github.com/heyu-ai/yibi-stack@v1.11.0"` | install 與三個 help 皆 exit 0；distribution 為 `yibi-stack`；不查找或 import checkout |
| SMK-002 | 驗證缺少 mycelium 時六個 skill 在工作前 fail-loud | SMK | High | plugin cache 可見六個 skill；PATH 無 `mycelium` | 依序啟動 `pr-cycle-fast`、`pr-control-log`、`pr-retrospective`、`mycelium`、`learn`、`local-port-manager` 的 preflight | 六個 skill；PATH 無 mycelium | 每個 skill 皆先印 `[FAIL]` 與 exact Git-tag install command、exit non-zero；未嘗試 `SKILL_REPO`、`uv run`、`uvx` 或 `python -m tasks.*` |
| SMK-003 | 驗證 hook registration 固定使用 PATH 解析出的 mycelium | SMK | High | clean HOME；empty `~/.claude/settings.json`；PATH 上的 installed binary 為 `/tmp/mycli-smoke/bin/mycelium` | 1. 記錄 `command -v mycelium` 2. 執行 `mycelium handover install-hooks` 3. 檢查 PreCompact 與 SessionStart command | recorded absolute mycelium path | 兩個 command 均以 `/tmp/mycli-smoke/bin/mycelium` 開頭並使用正確 hook subcommand；不含 checkout path、`python -m tasks.mycelium` 或 `uvx` |
...
[ADDED] Matrix 追加三個 proposal smoke slugs；此表是唯一 traceability source，proposal.md 不重複矩陣。
