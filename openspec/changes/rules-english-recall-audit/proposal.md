...
> 版本：v2.0 | 日期：2026-05-24 | 狀態：In Progress（PR-A #48 MERGED, PR-B #50 MERGED, PR-C/D 未啟動）
...
### PR-A #48 ✅ MERGED 2026-05-23：audit log fix + /recall + pre-commit gate
...
### PR-B #50 ✅ MERGED 2026-05-23：promotion gate + rule 結構調整
...
2. **rule 合併**：rule 14（shell-quoting-hygiene）內容合併到 rule 13（bash-anti-patterns）；rule 12（auto-handover）移至 `docs/rules-reference/auto-handover.md`（hook 已自動化，rule 為純參考文件）

### PR-C ⏳ NOT STARTED：6 條無 globs rule 英文化
...
### PR-D ⏳ NOT STARTED：plugins SKILL.md body 英文化（分次）
