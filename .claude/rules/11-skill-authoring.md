...
實證：openab_workspace PR #73 retro Q5「lesson 寫入規則文件」原本只列了 4 個 fallback 選項
（A/B/C/D）讓使用者選；對應路徑都列了具體檔案（pr-test-analyzer SKILL.md FAQ /
~/.claude/CLAUDE.md / yibi-stack 的 testing rule），所以使用者立刻能選 A 並一次完成落地。
反例：若 Q5 只寫「考慮把 lesson 寫進文件」，使用者選後還要再開一輪「寫哪？」對話，retro 落地
率會跌一個量級。
...
<!-- 正確：明確命名目的地 + 動詞 -->
- [ ] 寫入 `.claude/rules/13-bash-anti-patterns.md` AP2 section（git workflow recovery）
