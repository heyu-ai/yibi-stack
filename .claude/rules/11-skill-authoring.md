...
SKILL.md 中展示「錯誤示範」的 bash 程式碼，必須用 ` ```text ` fencing，不能用 ` ```bash `。

`scripts/lint_skill_bash.py` 的 `BASH_FENCE` regex 掃描所有 ` ```bash ` 區塊（含教學 demo），
用 ` ```bash ` 的教學違規示範會被 lint 當成真實 bash 檢查，產生假陽性失敗。

| 區塊用途 | fencing | lint 行為 |
|---------|---------|----------|
| 錯誤示範（教學用） | ` ```text ` | 跳過，不 lint |
| 正確範例（可執行） | ` ```bash ` | 掃描，報告違規 |

實證：`plugins/bash-hygiene/skills/bash-anti-patterns/SKILL.md` 的 AP1 violation demo（multi-line python -c、jq single-quoted filter）原本在 ` ```bash ` 中，導致 `lint_skill_bash.py --fail` 失敗。改為 ` ```text ` 後 lint 通過，教學語意保留。
