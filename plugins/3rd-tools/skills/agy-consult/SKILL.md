...
| exit | 意義 | 轉告重點 |
|------|------|----------|
| 124 | agy 在時間預算內沒完成（agy 本身回 exit 0 + 半截輸出，腳本擋下） | 問題範圍太大需大量探索檔案，或 API 額度不足；附上 `RESOURCE_EXHAUSTED` 那行（若有） |
| 1 | agy 回空白或極短輸出 | 照 stderr 的原因（常見：headless 權限自動拒絕、額度不足） |
| 2 | `AGY_PRINT_TIMEOUT_SECS` 不是整數秒 | 改成整數，例如 `480` |
