...
比 `/pr-review-cycle-mob` 輕量，不做 R2 cross-debate，適合快速拿 Gemini 第二意見。
...
> **執行說明**：腳本內部處理 `cd` 到 repo root（agy workspace sandbox 要求 `@相對路徑` 搭配 `--add-dir .`），完成後自動清理臨時 prompt 檔。直接執行即可，不要外加 log capture。
