...
改用 `make install` 建立的 symlink 定位。這個 symlink 暫時只保留 bootstrap resolver 相容 lane，
tasks-backed 操作一律呼叫 PATH 中 installed `mycelium`，不再從 checkout import `tasks/mycelium`。
目前不走 `CLAUDE_PLUGIN_ROOT`（實測該變數在 Bash tool 環境未設定）：
...
Script stdout 輸出 `KEY=VALUE`，agent 解析並記住。bootstrap 暫時仍輸出 `SKILL_REPO`
作為 resolver 相容 lane，但後續 mycelium 呼叫不依賴這個值：

- `SKILL_REPO` — 暫時保留的 resolver 相容值
