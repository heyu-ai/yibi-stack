#!/usr/bin/env bash
# pr-review-cycle-mob Step 3.2 — Gemini R1 Stage 2：Extract（raw → JSON）
#
# 用法：
#   bash ~/.agents/skills/pr-review-cycle-mob/scripts/gemini-r1-stage2.sh
#
# 刻意用 gemini-2.5-flash（低成本快速模型），避免再消耗 pro 配額。
#
# 副作用：
...
if ! gemini -m gemini-2.5-flash -p "@.pr-review/gemini-extract-input.md" \
    > "$REVIEW_DIR/gemini-r1.json" \
    2>"$REVIEW_DIR/gemini-r1.extract.log"; then
    echo "[FAIL] gemini extract 失敗，請查看 $REVIEW_DIR/gemini-r1.extract.log" >&2
...
echo "Gemini R1 Stage 2 complete"
