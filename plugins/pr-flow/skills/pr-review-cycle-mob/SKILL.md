...
> - **@file 觸發 agentic 模式（PR #303 教訓）**：`gemini -p "@/abs/path" > out.md` 在 redirect 模式下
>   可能讓 Gemini 進入 agentic tool call 模式——模型輸出 `call:read_file{...}` 文字而非實際 review 內容。
>   確認方式：若 `gemini-r1-raw.md` 開頭含 `call:` 或 `tool_use:` → 已觸發。
>   修法：在 `gemini` 指令加上 `--yolo` flag（略過工具執行確認框，強制輸出文字）；
>   或改用 worktree root 的相對路徑（`@.pr-review/gemini-r1-input.md`）而非絕對路徑。
