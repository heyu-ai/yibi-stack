...
`SKILL_REPO` 供後續 Codex / Gemini voice 的 extract step 使用，解析一次即可。
...
| Codex extract 後 JSON parse 失敗 | lead 偵測 `codex-r1.json` 非合法 JSON 時：警告使用者 + `cp /tmp/pr-review/codex-r1-raw.md /tmp/pr-review/codex-r1.md`；在 final.md 標註「Codex voice 本輪走 raw form，主 context 較重」 |
| Gemini extract 後 JSON 不符 schema | 同上，fallback `cp /tmp/pr-review/gemini-r1-raw.md /tmp/pr-review/gemini-r1.md`；final.md 同樣標註 |
