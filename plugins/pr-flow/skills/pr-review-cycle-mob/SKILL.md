...
| Gemini `gemini -p "@/tmp/..."` 失敗：path not in workspace | Gemini CLI `@file` 沙箱只允許讀取 worktree root 或 `~/.gemini/tmp/<worktree-name>/`，不能讀 `/tmp`；把 R1/R2 input 檔改寫到 worktree 目錄或 `~/.gemini/tmp/<worktree-name>/`，並確認 prompt 內容不含 `/tmp/` 路徑引用 |
