...
> **[重要] bash block 執行原則**：
> - 逐字執行下方 bash block，exit code 統一用 `if [ $? -ne 0 ]`，**禁止加 `echo "exit:$?"`** —— `$?` 在雙引號內觸發 `simple_expansion` 確認框
> - **Vertex AI 模式**（`GEMINI_AUTH: VERTEX_AI_OK`）：`GOOGLE_CLOUD_PROJECT` 已在 session env，直接執行 gemini 即可。若需補 `GOOGLE_CLOUD_LOCATION`，用 inline env：`GOOGLE_CLOUD_LOCATION=us-central1 gemini ...`，不要用獨立 `export` bash call
