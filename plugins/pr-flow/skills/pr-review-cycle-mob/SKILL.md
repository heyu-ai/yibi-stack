...
用 Write tool 把 review prompt 寫到 `$REVIEW_DIR/prompt-r1.md`（`$REVIEW_DIR` 為上方推導的實際路徑，如 `/tmp/pr-review/yibi-stack`）。寫入前先把 `{{REVIEW_DIR}}` 替換為 `$REVIEW_DIR` 的實際值：
...
> **[重要] bash block 執行原則**：
> - 逐字執行下方 bash block，**禁止在 gemini 指令後加任何 `$?` 相關程式碼**（包含 `echo "exit:$?"` 或額外的 `if [ $? -ne 0 ]`）—— Rule 5：parser 攔截 ALL `simple_expansion` 節點，`$?` 無論是否在引號內皆觸發確認框；bash block 已內含 exit code 判斷，不要另加
> - **Vertex AI 模式**（`GEMINI_AUTH: VERTEX_AI_OK`）：若當前 shell 仍有 `GOOGLE_CLOUD_PROJECT`（可用 `echo $GOOGLE_CLOUD_PROJECT` 確認），直接執行 gemini 即可。若需補 `GOOGLE_CLOUD_LOCATION`，用 inline env：`GOOGLE_CLOUD_LOCATION=<your-region> gemini ...`，不要用獨立 `export` bash call
...
codex exec -C "$WT" -s read-only -c 'model_reasoning_effort="high"' < "$REVIEW_DIR/codex-r2-input.md" > "$REVIEW_DIR/codex-r2.md" 2>&1
```
...
gemini -m gemini-3.1-pro-preview -p "@$REVIEW_DIR/gemini-r2-input.md" > "$REVIEW_DIR/gemini-r2.md" 2>&1
```
