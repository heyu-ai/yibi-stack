...
if [ "${PIPESTATUS[0]}" -ne 0 ]; then echo "[FAIL] codex review 失敗，請查看 $REVIEW_DIR/codex-r1.stage1.log"; exit 1; fi
...
if [ "${PIPESTATUS[0]}" -ne 0 ]; then echo "[FAIL] codex extract 失敗，請查看 $REVIEW_DIR/codex-r1.extract.log"; exit 1; fi
...
codex exec -C "$WT" -s read-only -c 'model_reasoning_effort="high"' < "$REVIEW_DIR/codex-r2-input.md" 2>&1 | tee "$REVIEW_DIR/codex-r2.md" > /dev/null
```
...
| Codex `codex review > file` 寫出 0 bytes（stderr 卻有完整 log） | Codex CLI 偵測 stdout 非 tty/非 pipe 時不輸出，file redirect 觸發此行為；本 skill 已改用 `\| tee file > /dev/null`，若客製需求請保留 pipe 結構，並用 `${PIPESTATUS[0]}` 取代 `$?` 檢查 codex exit code |
