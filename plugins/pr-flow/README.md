...
|-----------|-------------|
| `pr-review-cycle` skill | 完整 PR 生命週期：PR 建立 → review → fix → CI → merge |
| `pr-review-cycle-mob` skill | Mob review：多模型（Codex / Gemini）並行 R1+R2 交叉 debate，適合大型 PR |
| `pr-review-cycle-codex` skill | Codex-assisted PR review 流程 |
...
| `/pr` command | 一鍵建立 PR（含 description 自動生成） |
