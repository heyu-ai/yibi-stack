...
Claude Code plugin for Spec-Driven Development: Spectra amplifier methodology,
figma-design-sync (Figma design context extraction + incremental sync), and OpenSpec
...
`spectra-amplifier` dispatches `problem-frames`, `event-storming`, and `qa-test-design` by skill
name at runtime. Projects that use those steps need `methodology@yibi-stack` installed alongside
`sdd@yibi-stack`.
