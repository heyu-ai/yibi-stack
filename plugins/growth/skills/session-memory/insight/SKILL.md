    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "uv run --project <repo> python -m tasks.session_memory insight collect"
          }
        ]
      }
    ]
...
```bash
uv run python -m tasks.session_memory insight list --last 1
```
...
```bash
uv run python -m tasks.session_memory insight uninstall-hook
```
