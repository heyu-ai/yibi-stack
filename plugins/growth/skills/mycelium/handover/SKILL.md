...
> 執行任何 `uv run python -m tasks.mycelium` 指令前，先跑 Step 1 的腳本
> 捕捉原始 project（worktree-safe）並解析 `SKILL_REPO`，
> 之後所有指令都帶 `--directory "$SKILL_REPO"`。
...
若 DB 不存在，先跑 `uv run --directory "$SKILL_REPO" python -m tasks.mycelium init`。

### Step 2 — 寫入交班

```bash
uv run --directory "$SKILL_REPO" python -m tasks.mycelium handover write \
...
uv run --directory "$SKILL_REPO" python -m tasks.mycelium handover read --last 3 --project "$ORIG_PROJECT"

# 不帶 --project 顯示所有專案的記錄（向後相容）
uv run --directory "$SKILL_REPO" python -m tasks.mycelium handover read --last 4

# 原始 JSON
uv run --directory "$SKILL_REPO" python -m tasks.mycelium handover read --last 4 --json
```
...
```bash
uv run --directory "$SKILL_REPO" python -m tasks.mycelium handover search --query "parser"
uv run --directory "$SKILL_REPO" python -m tasks.mycelium handover search --query "bug" --type debug --limit 5
uv run --directory "$SKILL_REPO" python -m tasks.mycelium handover search --project flight-mcp
uv run --directory "$SKILL_REPO" python -m tasks.mycelium handover search --account claude-pro
```
...
| 問題 | 解法 |
|------|------|
| 找不到 DB | 先跑 `uv run --directory "$SKILL_REPO" python -m tasks.mycelium init` |
