...
```bash
# 列出最近 10 筆（預設）
uv run python -m tasks.mycelium recap list

# 過濾特定 project
uv run python -m tasks.mycelium recap list --project yibi-stack

# 查單一 session 的時序軌跡
SESSION_ID="4c6cec93-a48c-4d8f-93b2-2ad43150c263"
uv run python -m tasks.mycelium recap list --session "$SESSION_ID"
...
echo "{\"hook_event_name\":\"Stop\",\"transcript_path\":\"$TRANSCRIPT\",\"reason\":\"\"}" | \
  uv run python -m tasks.mycelium recap collect
...
```bash
uv run python -m tasks.mycelium recap uninstall-hook
```
