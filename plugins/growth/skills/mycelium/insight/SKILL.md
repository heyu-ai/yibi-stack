...
```bash
cat ~/.claude/settings.json | python3 -c "import json,sys; print(json.load(sys.stdin).get('hooks',{}).get('Stop', '(未設定)'))"
```
...
```bash
uv run python -m tasks.mycelium insight list --last 1
```
...
```bash
uv run python -m tasks.mycelium insight uninstall-hook
```
...
```bash
# CLI 列出最近 N 筆
uv run python -m tasks.mycelium insight list --last 10
uv run python -m tasks.mycelium insight list --project yibi-stack
