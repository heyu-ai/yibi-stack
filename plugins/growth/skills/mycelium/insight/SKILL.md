...
> 執行任何 `uv run python -m tasks.mycelium` 指令前先解析 `SKILL_REPO`，
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "uv run --project <repo> python -m tasks.mycelium insight collect"
          }
        ]
      }
    ]
...
```bash
uv run --directory "$SKILL_REPO" python -m tasks.mycelium insight list --last 1
```
...
```bash
uv run --directory "$SKILL_REPO" python -m tasks.mycelium insight uninstall-hook
```
...
```bash
# CLI 列出最近 N 筆
uv run --directory "$SKILL_REPO" python -m tasks.mycelium insight list --last 10
uv run --directory "$SKILL_REPO" python -m tasks.mycelium insight list --project yibi-stack
