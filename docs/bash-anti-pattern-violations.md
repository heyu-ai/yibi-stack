...
`skills/mycelium/` handover skill 改用 `| xargs basename` 等不含 quoted arg 的寫法。
...
uv run --directory "$SKILL_REPO" python -m tasks.mycelium handover read --last 1 --full
```
...
純 ASCII 不觸發 AP2；已在 `skills/mycelium/SKILL.md` 和 `skills/verify-gemini-models/SKILL.md` 實戰驗證。

**連帶修正**（本次 PR）：

- `commands/handover.md`（3 個 bash 區塊）
- `commands/handover-back.md`（2 個 bash 區塊）
- `skills/mycelium/SKILL.md`（2 處）
