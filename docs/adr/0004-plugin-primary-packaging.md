...
- **版本落差取代路徑落差。** 使用者安裝的 CLI 可能落後 plugin 的 SKILL.md。每個 skill 都必須
  加上能力檢查並 fail-loud。這比今天「靜默解析到錯 repo」是**更好的失敗**，但它是新增
  的失敗模式。**這是最重要的一條**：若少了它，Phase 2/3 等於把大聲的路徑失敗換成安靜的行為
  不一致——嚴格來說比現狀更糟。

  > **修訂（2026-07-16，PR #249 mob review）**：本條原文寫的是「能力／**版本**檢查」，並在
  > 下方「跨階段議題」指定用 `<cli> --version` 比對最低版本。**該機制在本 ADR 自己指定的
  > 安裝方式下做不到**，經三家 reviewer（Codex / agy / Claude）獨立收斂確認：
  >
  > `uv tool install git+https://github.com/heyu-ai/yibi-stack` 安裝的是 **HEAD**，但其
  > metadata 版本字串取自 `pyproject.toml`，也就是**上次 release** 的值。兩次 release 之間
  > 的每個 commit 都回報同一個版本，所以 semver 比較**偵測不到它要偵測的漂移**——它是一道
  > PASS 不帶資訊的閘門，正是本 repo 規則（rule 09）明文禁止的形狀。
  > 提出此要求的 reviewer（Codex）在 R2 debate 中主動撤回自己的 finding，理由同上。
  >
  > **修正後的立場**：
  > - `--version` 的定位是**診斷**（人看的、bug report 用的），**不是閘門**。
  > - 閘門是**能力檢查**：指令存在（`command -v <cli>`）、安裝未損毀（`<cli> --version`
  >   非零退出即 fail-loud）。Phase 1 的 `portman` 即採此形式。
  > - 若日後真的需要相容性閘門，必須用**穩定的 capability/protocol revision 或具體行為
  >   probe**（例如 `<cli> capabilities` 輸出的能力集合），而非 package semver。該機制
  >   **尚未設計**，且應在 Phase 2 有 skill 真正需要版本專屬功能時才設計——在此之前預先
  >   加一道恆真的版本比較，只會製造虛假的安全感。
  >
  > 這個修訂的價值在於**阻止 Phase 2/3 照抄一個做不到的模式**。實作細節見
  > [plugin-primary-plan.md](../plugin-primary-plan.md) 的「跨階段議題」。
