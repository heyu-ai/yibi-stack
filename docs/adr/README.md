...
| **改變決定實質**（推翻、替換機制、調整範圍） | 一律**開新 ADR**。新 ADR frontmatter 用 `supersedes`（整份取代）或 `supersedes_clause`（取代某段）指向舊 ADR；舊 ADR 底部加 `## Superseded by` 反向連結。舊 ADR `status` 視情況改 `superseded`（整份）或維持 `accepted` 並於該段旁註記被哪份 ADR 的哪條取代（部分）。 |

反例：ADR-0004 曾被 in-place 改動實質、之後又還原，過程無跡可循——正是此慣例要避免的。
