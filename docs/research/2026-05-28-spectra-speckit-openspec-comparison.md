...
Codex：fork-context read-only skills 是工具隔離（tool isolation），Teddy L3 是基於 Bounded Context 的業務領域分工。
Gherkin/QA subagent 屬 L1 角色，不是 L3 bounded-context agent。建議修正為「Spectra 在本地 context 隔離上領先，但 Bounded Context 尚未第一公民化（G9 仍是未來項目），L3 尚未完整落地」。
...
Codex web 搜尋確認 OpenSpec 目前 core profile 含 `propose / explore / apply / sync / archive`，擴展包含 `verify / bulk-archive / onboard`。
文件中「無 drift 偵測」（Part 4.1 / 表格第 34 行）的說法因 `/opsx:verify` 可檢查 artifact 一致性而被削弱；`/opsx:sync` 也具備 delta spec 合併能力。
...
> **編輯注**：agy 另提及「Spectra SDD skill 實際為 9 個、使用點號命名（spectra.tasks 等）」，此判斷與本 repo 實際架構不符——`.claude/skills/` 下的 12 個 spectra-* skill 使用連字號命名，
> 與 Spec Kit `/speckit.*` 命令體系是不同的系統。推測 agy 將 Spec Kit 命令對應表的 9 項混入了判斷，屬 reviewer 資訊邊界限制，不採納此點。
