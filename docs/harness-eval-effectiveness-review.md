...
>
> TL;DR 第 3 點「活證據」的**行數**是修正前的數字（同組數字亦見於 C-1、A-1、A-3）；修正後
> always-loaded 集合為 01/02/03/13/15/16 共 6 個檔案。**但 D7 的 6/7 不受本次修正影響**——
> 該 scanner 偵測的是 `glob:` 子字串，`globs:` 與 `paths:` 兩種拼法都不匹配，分數一直來自
> `elif len(rule_files) >= 3` 的 fallback 分支，與 frontmatter 無關（見 issue #252）。
> 本文「D7 把 rules 數量當品質加分、對 token 成本零偵測」的論點在修正後依然成立。
> 本文原文一字未改，僅加註此更正。
