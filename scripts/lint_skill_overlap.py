...
- 先剝除 slash-prefixed skill 參照（「請改用 /X」「退回 /X」「與 /X 共用」）：被導向的兄弟
  skill 名是 over-trigger 的『解法』而非觸發詞，計入會讓互相 redirect 的 pair 因彼此的名字虛高。
  錨定 `/skill` 才剝，避免把裸「改用/退回/請用」等日常動詞連同 domain 觸發詞一起吃掉。
...
# Slash-prefixed skill 參照（`/codex-review`、`` `/pr-cycle-fast` ``、`/agy`…）宣告的是
# 「不要用本 skill、改用被導向的兄弟 skill」——它是 over-trigger 的『解法』，不是本 skill 的
# 觸發詞。若把被導向的兄弟 skill 名算進關鍵字，兩個互相 redirect 的 skill 會因為彼此的名字
# 成為共享 token 而 Jaccard 虛高，等於懲罰 rule 11 指定的正確修法（作者被逼去刪 redirect
# 子句來讓 lint 過關）。故 tokenize 前先剝除這些 skill 參照，連同緊鄰其前的 redirect 標記
# （請改用 / 改用 / 退回）一起剝掉，避免標記 bigram 也殘留成共享詞。
#
# **必須錨定 `/skill` 才剝除**（rule 13「exemption regex 要精確枚舉、不用 open glob」）：裸的
# 「改用」「退回」「請用」是日常中文動詞（「改用 polars」「退回上一個 commit」「請用參數化查詢」），
# 若無 `/` 錨定就會連 domain 觸發詞一起吃掉，反而讓真實 over-trigger 逃過偵測（lint 是 warn-only，
# 這種假陰性會無聲漏報）。負向 lookbehind `(?<![A-Za-z0-9])` 排除 `CI/CD` 這種字母間的斜線。
# marker 設為可選：逗號分隔的多目標清單（`請改用 /a、/b`）第二個目標前沒有 marker，仍需剝除。
_REDIRECT_CLAUSE_RE = re.compile(
    r"(?:請改用|改用|退回)?\s*(?<![A-Za-z0-9])`?/[A-Za-z][A-Za-z0-9_-]*"
)
...
def extract_keywords(description: str) -> set[str]:
    ...
    先剝除 slash-prefixed skill 參照（見 `_REDIRECT_CLAUSE_RE`）——被導向的兄弟 skill
    名是『解法』不是觸發詞，計入會讓互相 redirect 的 pair 虛高；錨定 `/skill` 才剝，
    裸的「改用/退回」等日常動詞不受影響。
    """
