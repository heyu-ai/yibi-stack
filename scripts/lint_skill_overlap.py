...
- 先剝除兄弟 skill 參照（`/codex-review`、`` `/pr-cycle-fast` `` 這類）：被導向的 skill 名是
  over-trigger 的『解法』而非觸發詞，計入會讓互相 redirect 的 pair 因彼此的名字虛高。剝除靠
  「名稱是不是真的 skill」判斷（`_build_skill_ref_re`），不靠猜前綴詞——故路徑/URL 段（`etc`、
  `api`）天生安全；另加 `請改用` 標記涵蓋非 scanned 的 CLI 指令（見 `_REDIRECT_MARKER_RE`）。
...
# 四輪 mob review 才收斂到這個形狀：任何靠「context word（請改用/改用/退回/與/、/或）+ /token」
# 猜測的做法，context word 本身也是日常中文詞，總有一個 URL / 絕對路徑 / 標點後路徑的反例被
# 誤剝（`https://api` 的 `/api`、`與 /etc/passwd`…），造成 domain 觸發詞流失、真實 over-trigger
# 逃過偵測。治本：**只剝除「已知 skill 名稱」的斜線參照**——路徑段（`etc`/`api`）永遠不是
# 已知 skill 名，故天生 path-safe。
#
# (1) `_SKILL_REF_RE`（在 main() 依實際掃到的 skill 名動態編譯，見 `_build_skill_ref_re`）：
#     剝除 `` `?/<已知 skill 名> `` ——涵蓋 redirect（`請改用 /codex-review`）、比較
#     （`與 /pr-cycle-deep 共用`）、清單（`/a、/b`）、個別反引號包裹（`` `/a`、`/b` ``），
#     不管前綴語境，因為靠「名稱是不是真 skill」判斷，不靠猜前綴。
...
def _build_skill_ref_re(skill_names: frozenset[str]) -> re.Pattern[str] | None:
    """依實際 skill 名編譯 `` `?/<name> `` 剝除 regex；名稱長者在前避免前綴誤匹配。

    回傳 None 表示沒有任何 skill 名（呼叫端據此略過 (1) 剝除）。字尾 `(?![\\w-])`
    確保 `/agy` 不會誤匹配 `/agyx`（word boundary）。
    """
    if not skill_names:
        return None
    alternation = "|".join(re.escape(n) for n in sorted(skill_names, key=len, reverse=True))
    return re.compile(r"`?/(?:" + alternation + r")(?![A-Za-z0-9_-])")
...
def extract_keywords(description: str, skill_ref_re: "re.Pattern[str] | None" = None) -> set[str]:
    """從 description 抽觸發關鍵字 set：ASCII word token + CJK bigram shingle。

    先剝除兄弟 skill 參照（被導向的 skill 名是 over-trigger 的『解法』不是觸發詞，計入會讓
    互相 redirect 的 pair 虛高）：
    - `skill_ref_re`（在 main() 依實際 skill 名編譯，見 `_build_skill_ref_re`）：剝除
      已知 skill 名的 `/參照`，path-safe（路徑段不是 skill 名）。None 時略過（如單元測試
      未提供已知集合）。
    - `_REDIRECT_MARKER_RE`：剝除 `請改用` 標記後的 /token 清單，涵蓋非 scanned 的 CLI 指令。
    """
    # 先套 marker（吃掉「請改用 + target 清單」整段，連 marker 詞一起，避免殘留「改用」bigram），
    # 再套 skill_ref（剝除 marker 以外語境的已知 skill 參照，如比較句「與 /X」）。
    description = _REDIRECT_MARKER_RE.sub(" ", description)
    if skill_ref_re is not None:
        description = skill_ref_re.sub(" ", description)
...
def main() -> int:
    ...
    # 先收集 (skill_name, description)，同時累積所有 skill 名——`_build_skill_ref_re` 用它
    # 剝除「已知 skill 名」的 /參照（見 extract_keywords）。必須先掃完全部名稱才能 tokenize，
    # 因為任一 description 可能 redirect 到任一其他 skill。
    ...
    for skill_name, skill_md in iter_global_skill_files(SKILLS_DIR, PLUGINS_DIR):
        ...
        if description is None:
            ...
            continue
        collected.append((skill_name, description))

    skill_ref_re = _build_skill_ref_re(frozenset(name for name, _ in collected))
    skills: list[tuple[str, set[str]]] = [
        (name, extract_keywords(desc, skill_ref_re)) for name, desc in collected
    ]
    checked = len(skills)
