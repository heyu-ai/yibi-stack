...
- CJK：對連續中文字元跑滑動視窗抽 bigram（2 字元 shingle），濾掉一份常見「樣板虛詞」
  ...
  pair 都「重疊」，訊號消失）。
- 兩份 keyword set 算 Jaccard similarity（交集 / 聯集）；超過 `--threshold` 才列為風險 pair。
...
Usage:
  ...
  python3 scripts/lint_skill_overlap.py --fail            # 有重疊超過門檻 -> exit 1
  python3 scripts/lint_skill_overlap.py --threshold 0.25  # 調整靈敏度（預設 0.12）

Exit code:
  0 -> 無重疊超過門檻，或未加 --fail（warn-only 模式恆回 0）
  1 -> --fail 模式且有重疊超過門檻
  2 -> 設定錯誤（skills 目錄缺失）
"""
...
_CJK_STOPWORDS = frozenset(
    {
        "觸發", "情境", "關鍵", "鍵字", "使用", "可以", "需要", "進行", "應該",
        "時候", "適用", "情況", "或是", "以及", "不要", "而是", "這個", "那個",
        "什麼", "怎麼", "如何", "是否", "已經", "目前", "的是", "也應", "也是",
        "才能", "只要", "只是", "但是", "因為", "所以", "如果", "當你", "當用",
        "用戶", "使者", "者說", "說「", "」時",
    }
)
...
def find_overlaps(
    ...
    """兩兩算 Jaccard；回傳超過門檻的 (name_a, name_b, score, shared_keywords) 依 score 降冪。"""
...
def main() -> int:
    ...
    if risky:
        print(
            f"[WARN] 偵測到 {len(risky)} 對觸發詞高度重疊的 skill"
            f"（Jaccard >= {threshold}）：",
            file=sys.stderr,
        )
        ...
        if fail_mode:
            return 1
        print("提示：用 --fail 旗標可讓此 script 在有重疊超過門檻時 exit 1", file=sys.stderr)
        return 0

    print(f"[OK] 已檢查 {checked} 個 skill，無觸發詞重疊超過門檻（Jaccard >= {threshold}）")
    return 0
