...
class PRCreator:
    """在 worktree 外建立 PR branch 並呼叫 gh pr create。"""
    ...
    def _git_commit(self, message: str) -> None:
        # Write to temp file to avoid multi-line message issues
        msg_path = PROJECT_ROOT / ".runtime" / "nightly_agent_commit_msg.txt"
        msg_path.parent.mkdir(parents=True, exist_ok=True)
        msg_path.write_text(message, encoding="utf-8")
        result = subprocess.run(  # nosec B603
            ["git", "-C", str(PROJECT_ROOT), "commit", "-F", str(msg_path)],
            capture_output=True,
            text=True,
            timeout=30,
        )
    ...
    def _build_pr_body(self, proposal: ArtifactProposal, test_result: TestResult) -> str:
        ...
        return (
            ...
            f"---\n*Generated by `tasks.nightly_agent` on {datetime.now().strftime('%Y-%m-%d %H:%M')}*"
        )

    def _gh_pr_create(self, branch: str, title: str, body: str) -> str:
        # Write body to temp file
        body_path = PROJECT_ROOT / ".runtime" / "nightly_agent_pr_body.txt"
