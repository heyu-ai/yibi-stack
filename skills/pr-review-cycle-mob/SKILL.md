...
description: >
  ...
  group review：自動偵測 codex / gemini / 本地 open-weights CLI，達 2 家以上即啟動
...
- 想快速合併、不想啟動多家模型
- 沒裝 codex / gemini / 本地 open-weights CLI

**核心理念**：當 codex / gemini / 本地 open-weights CLI 偵測得到時，把它們和 Claude
開成**同步平行**的 reviewer 群，先各自獨立評審，再交叉看彼此意見 debate，產出 aggregated
最終報告。Coding agent（Claude main session）按報告修改，再開一輪 group review，循環
到全員 LGTM（含 actionable NIT）為止。最後人類花幾分鐘掃一眼所有改動，當場提出疑慮，由
...
執行下列獨立 bash calls 偵測各 reviewer 的可用性。每個 call 簡短、彼此獨立。
...
test -n "$CODEX_API_KEY" -o -n "$OPENAI_API_KEY" && echo "CODEX_AUTH: KEY_SET" || true
```
...
```bash
# Open-weights：env override 優先
test -n "$OPENWEIGHTS_REVIEWER_CMD" && echo "OW: ENV_CMD_SET" || echo "OW: ENV_CMD_UNSET"
```

```bash
# Open-weights：ollama 本地推論（無需 API key，diff 不出機器）
which ollama >/dev/null 2>&1 && echo "OW_OLLAMA: BINARY_OK" || echo "OW_OLLAMA: NOT_FOUND"
```

```bash
# Open-weights：列出已 pull 的 code-capable 模型（qwen / deepseek / codestral 任一即可）
ollama list 2>/dev/null | awk 'NR>1{print $1}' | grep -Ei 'qwen|deepseek|codestral|llama.*code' | head -3
```

### 模式判定

依「外部 reviewer 計數」決定流程。Mob skill 要求**至少 2 家外部模型**才有 mob 群審
意義，1 家或 0 家都退回 `/pr-review-cycle`：

| 外部 reviewer 數 | 動作 |
|---:|---|
| 0 | **退回 `/pr-review-cycle`**（Claude-only 即足夠；本 skill 終止） |
| 1 | **退回 `/pr-review-cycle`**（單家 reviewer 不算 mob，仍建議 Claude-only 即可；若仍想保留該外部 voice 當 gate，使用 [DEPRECATED] `/pr-review-cycle-codex` 的 codex-only 流程） |
| **2** | **mob review**（本 skill 主流程，2 外部 voice + Claude） |
| **3** | **full mob review**（本 skill 主流程，3 外部 voice + Claude，覆蓋最廣） |

每個外部 reviewer「可用」需 binary OK + auth OK 兩條件。Open-weights 額外允許：
`$OPENWEIGHTS_REVIEWER_CMD` 設定（ENV_CMD_SET）即視為可用，否則需 ollama BINARY_OK 且
有 code 模型已 pull。
...
- Codex         ✓ / ✗
- Gemini        ✓ / ✗
- Open-weights  ✓（透過 {{ollama|env-cmd}}）/ ✗

外部 reviewer 計數：{{N}}/3
模式：{{mob-review-2voice|full-mob-3voice|REDIRECT}}
...
`-C` 防 CWD 漂移（AP3 Sub-class A）。`--base` 與 positional prompt 互斥，所以
不能再加 prompt 字串；codex 內建 review 模式自動產生 [P1]/[P2] 分級，lead 後續轉成
Critical/Important。

##### Gemini voice（GEMINI_OK 時）

Gemini CLI 不接受 stdin prompt + diff path 的多檔組合，先把 diff + prompt 串成單一檔：
...
env 模式（`$OPENWEIGHTS_REVIEWER_CMD` 為 stdin → stdout 的 wrapper）：

```bash
cat /tmp/pr-review/prompt-r1.md /tmp/pr-review/diff.patch | $OPENWEIGHTS_REVIEWER_CMD > /tmp/pr-review/openweights-r1.md 2>&1
```

ollama 模式（預設 `qwen2.5-coder:32b`，可用 `OPENWEIGHTS_OLLAMA_MODEL` 覆蓋）：

```bash
cat /tmp/pr-review/prompt-r1.md /tmp/pr-review/diff.patch | ollama run "${OPENWEIGHTS_OLLAMA_MODEL:-qwen2.5-coder:32b}" > /tmp/pr-review/openweights-r1.md 2>&1
```
...
用 Write tool 把所有 R1 內容串成 `/tmp/pr-review/r1-aggregate.md`：
...
## Open-weights ({{model}})
<貼 /tmp/pr-review/openweights-r1.md>
```
...
```bash
cat /tmp/pr-review/prompt-r2.md /tmp/pr-review/r1-aggregate.md > /tmp/pr-review/<voice>-r2-input.md
```
...
```bash
$OPENWEIGHTS_REVIEWER_CMD < /tmp/pr-review/openweights-r2-input.md > /tmp/pr-review/openweights-r2.md 2>&1
```
...
## Mode
group-review（{{N}}/4 voices active）
...
Group review 模式：{{N}}/4 voices LGTM（Claude / Codex / Gemini / Open-weights）。
```
...
| Voice | 偵測 | R1 / R2 呼叫模板 | 預期輸出 |
|---|---|---|---|
| Claude | always | Task() pr-review-toolkit 4 subagents | finding markdown |
| Codex | `which codex` + auth | `codex review -C $REPO --base $BASE` (R1) / `codex exec -C $REPO -s read-only < input.md` (R2) | [P1]/[P2] 分級 |
| Gemini | `which gemini` + auth | `gemini -m gemini-3.1-pro-preview -p @input.md` | 散文格式（lead 解析）|
| Open-weights | `$OPENWEIGHTS_REVIEWER_CMD` 或 ollama + code 模型 | `ollama run $MODEL < input.md` 或 `$CMD < input.md` | 散文格式 |
...
| Step 0 只偵測到 1 家可用 | 本 skill 終止，改執行 `/pr-review-cycle`；想保留該 voice 當 gate 用 [DEPRECATED] `/pr-review-cycle-codex` |
| 偵測到 codex 但 auth 失敗 | `codex login`；或 `export OPENAI_API_KEY=...` |
| 偵測到 gemini 但 auth 失敗 | `gemini auth login`；或 `export GEMINI_API_KEY=...` |
| ollama 沒有 code 模型 | `ollama pull qwen2.5-coder:32b`；或設 `$OPENWEIGHTS_REVIEWER_CMD` 改用其他 wrapper |
...
| `/pr-review-cycle-mob`（本 skill）| 中大型 PR / 高風險改動 / 跨家視角壓力測試 | Claude + ≥2 家外部 frontier model（codex / gemini / open-weights）R1 獨立 + R2 debate |
| `/pr-review-cycle-codex` [DEPRECATED] | 只裝 codex 一家、想當硬性 gate 用 | Claude + codex（codex-only 強化版） |

本 skill 強制要求 ≥2 家外部 reviewer 才啟動，否則退回 `/pr-review-cycle`，避免
「mob」名稱與實際只有單一外部 voice 的語意衝突。
