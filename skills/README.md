...
| `freeze` | tool | [plugins/harness/](../plugins/harness/README.md) | 把編輯範圍鎖定到指定目錄：PreToolUse hook 擋掉邊界外的 Edit/Write，防止 agent 順手改到不相關的檔案。未設邊界前 fail-open（inert）。改寫自 garrytan/gstack（MIT），state 改存 `~/.agents/` | [freeze/SKILL.md](freeze/SKILL.md) |
| `unfreeze` | tool | [plugins/harness/](../plugins/harness/README.md) | 清除 `/freeze` 設下的編輯邊界，恢復全目錄可編輯。改寫自 garrytan/gstack（MIT） | [unfreeze/SKILL.md](unfreeze/SKILL.md) |
