...
| `pr-cycle` | exec | [plugins/pr-flow/](../plugins/pr-flow/README.md) | PR 生命週期自動化 orchestrator：偵測 open PR → 並行 code review + CI monitor + conflict detect → auto-fix markdownlint/CI（≤3 次）→ merge → /pr-retro 寫 mycelium → /clean-merged。State machine 可中斷 resume | [pr-cycle/SKILL.md](pr-cycle/SKILL.md) |
