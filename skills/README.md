...
| `agy-review` | tool | [plugins/3rd-tools/](../plugins/3rd-tools/README.md) | Antigravity CLI（agy）對 diff 做 code review（PASS/FAIL gate）或 challenge（對抗模式找 bug/security）；不啟動 mob 流程的輕量單一 reviewer，全程 `--sandbox`。**預設模型 `claude-sonnet-4-6`，不是 Gemini**，故不可逕自計為跨廠商聲音 | [agy-review/SKILL.md](agy-review/SKILL.md) |
