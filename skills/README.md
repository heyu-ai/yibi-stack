...
| `pr-review-cycle-codex` | [DEPRECATED] codex-only 強化版；想要 mob 群審用 `/pr-review-cycle-mob`，小型 PR 用 `/pr-review-cycle`。住址：[plugins/spectra/](../plugins/spectra/README.md) | [pr-review-cycle-codex/SKILL.md](pr-review-cycle-codex/SKILL.md) |
| `bash-anti-patterns` | Claude Code agent 下 bash 指令三層防線：AP1 過度複雜單行 / AP2 bash 字串 Unicode / AP3 stateful cd；Rule 14 shell 引號衛生；Rule 15 不可逆操作邊界；含判斷標準、對策決策樹與可選裝 PreToolUse hook | [bash-anti-patterns/SKILL.md](bash-anti-patterns/SKILL.md) |
