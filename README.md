...
skills/           Agent execution layer -- SKILL.md runbooks
  global/         Works in any repo: methodology, cross-project tools
  project/        Requires this repo's Python tasks (gmail, scheduler, etc.)
...
### Is it a Claude Code plugin?

**Yes -- partially.** `plugins/bash-hygiene` and `plugins/spectra` are proper Claude Code plugins with `package.json` manifests, installable via `claude plugin install` without cloning the repo.

The skills layer requires cloning and running `make install`, which symlinks SKILL.md runbooks into `~/.claude/skills/` and `~/.agents/skills/`.
...
skills/           Agent 執行介面層（SKILL.md runbook）
  global/         跨專案可用：方法論、通用工具
  project/        本 repo 限定（需要 tasks/ 的 Python 實作）
...
### 這是一個 Claude Code plugin 嗎？

**是的，但有兩種層面。**

`plugins/bash-hygiene` 和 `plugins/spectra` 是有 `package.json` manifest 的正式 Claude Code plugin，可以直接透過 `claude plugin install` 安裝，不需要 clone 整個 repo。

Skills 層則需要 clone 並執行 `make install`，將 SKILL.md runbook 以 symlink 安裝到 `~/.claude/skills/` 和 `~/.agents/skills/`。
